...
class UpdateDB extends \Magento\Framework\App\Helper\AbstractHelper
{
   ...
   public $url = 'https://rinkeby.infura.io/fNuraoH3vBZU8d4MTqdt';
    ...
    public function __construct(
        ...
        \Fredo\Nexty\Helper\Blockchain $blockchain
    ) {
    ...
    private function get_connection_db(){
    ...
    private function get_exchange_table_name(){
      return $this->db_prefix.'exchange_to_usd';
    }
    ...
    private function get_order_id_from_input($input_hash){
      ...
      $input=($this->hexToStr($input_hash))."<br>";
      //echo $input;
...
////////////////////////change to PRIVATE after TESTING
    ...
    private function insert_transactions_db($connection,$transactions,$transactions_table_name,$admin_wallet_address,$block_time){
      ...
      if (($transaction['to']) && (strtolower($transaction['to'])==strtolower($admin_wallet_address)))
      {
        ...
        $order_id=$this->get_order_id_from_input($transaction['input']);

        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $order = $objectManager->create('\Magento\Sales\Model\Order') ->load($order_id);
        if ($order->getId()) {return;}
        //$order_total=$order->getGrandTotal(); ///SHOP CURRENCY
        //$paid_sum=$this->get_paid_sum_by_order_id($connection,$transactions_table_name,$order_id); //NEXTY COIN

        $block_number_dec= hexdec($block_number);

        if (!$this->transaction_exist($connection,$transactions_table_name,$hash)){
    ...
    public function place_order_in_coin($order_id, $order_total, $order_currency){
    ...
    //	$exchange_table_name	= $this->get_exchange_table_name();
    	...
    	$start_block_number=2332666; //testing transaction at 2285555
